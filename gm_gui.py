...
class GroundMotionProcessing():
    ...
    def __init__(self, master):
        ...
        self.fig1, self.ax1 = plt.subplots(nrows=1, ncols=1, figsize=(5, 3))
        ...
        self.fig2, self.ax2 = plt.subplots(nrows=1, ncols=1, figsize=(5, 3))
    ...
    def update_fseries_plot(self):
        ...
        # self.ax2.set_xticks([0.01, 0.03, 0.1, 0.3, 1, 3, 10, 100, 300, 1000])
    ...
    def init_layout(self):
        ...
        self.master.minsize(6, 8)

        # Frame 1 ----------------------------------------------------
        frame1 = Frame(bg=frame_bg)
        frame1.grid(row=0, column=0)

        title_1 = Label(frame1, text="Time Domain Settings",
        ...
        # Run / Undo / New File
        # subtitle_1_3 = Label(frame1, text="Apply Changes",
        #                      bg=label_bg, font=subtitle_font)
        # subtitle_1_3.grid(row=9, columnspan=2)
        ...
        title_3 = Label(frame3, text="Frequency Domain Settings",
                        bg=label_bg, font=title_font)
        title_3.grid(row=0, columnspan=2)

        # Smoothing
        subtitle_3_1 = Label(frame3, text="Konno and Ohmachi Smoothing",
                             bg=label_bg, font=subtitle_font)
        subtitle_3_1.grid(row=1, columnspan=2)

        label_3_1_2 = Label(frame3, text="Bandwidth, b (#):", bg=label_bg)

        self.apply_smoothing = BooleanVar()
        self.smooth_width = StringVar()
        input_3_1_1 = Checkbutton(frame3, text="Apply Smoothing",
        ...
        input_3_1_1.grid(row=2, columnspan=2)
        input_3_1_2.grid(row=3, column=1)

        # Resampling
        subtitle_3_2 = Label(frame3, text="Resample",
                             bg=label_bg, font=subtitle_font)
        subtitle_3_2.grid(row=4, columnspan=2)
        ...
        input_3_2_1 = Checkbutton(frame3, text="Resample",
                                  variable=self.apply_resampling, 
                                  bg=check_bg, disabledforeground=check_bg)
        ...
        input_3_2_1.grid(row=5, columnspan=2)
        input_3_2_2.grid(row=6, column=1)
        input_3_2_3.grid(row=7, column=1)
        input_3_2_4.grid(row=8, column=1)

        # Run / Undo / New File
        # subtitle_3_3 = Label(frame3, text="Apply Changes",
        #                      bg=label_bg, font=subtitle_font)
        # subtitle_3_3.grid(row=9, columnspan=2)

        input_3_3_1 = Button(frame3, command=self.edit_fseries,
                             text="Apply", bg=button_bg)
        # input_3_3_2 = Button(frame1, command=self.reload_file,
        #                      text="Undo", bg=button_bg)

        input_3_3_1.grid(row=10, columnspan=2)
        # input_3_3_2.grid(row=11, columnspan=2)
        # input_3_3_3.grid(row=12, columnspan=2)
        # input_3_3_4.grid(row=13, columnspan=2)
    ...
    def edit_tseries(self):
        ...
        if self.apply_taper.get() and not self.applied_taper:
            try:
                self.tseries.cosine_taper(width=float(self.cos_width.get())/100)
    ...
    def edit_fseries(self):
        ...
        if self.apply_smoothing.get() and not self.applied_smoothing:
            ...
            try:
                self.fseries.smooth_konno_ohmachi(float(self.smooth_width.get()))
    ...
    def load_new_file(self):
        ...
        resp = filedialog.askopenfilename(
